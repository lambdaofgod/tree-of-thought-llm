...
from tot.models import llm_generate
...
def get_value(task, x, y, n_evaluate_sample, cache_value=True):
    ...
    value_outputs = llm_generate(value_prompt, n=n_evaluate_sample, stop=None)
...
def get_votes(task, x, ys, n_evaluate_sample):
    ...
    vote_outputs = llm_generate(vote_prompt, n=n_evaluate_sample, stop=None)
...
def get_proposals(task, x, y):
    ...
    proposals = llm_generate(propose_prompt, n=1, stop=None)[0].split('\n')
    return [y + _ + '\n' for _ in proposals]
...
def get_samples(task, x, y, n_generate_sample, prompt_sample, stop):
    ...
    samples = llm_generate(prompt, n=n_generate_sample, stop=stop)
...
def solve(args, task, idx, to_print=True):
    ...
    llm_generate = partial(llm_generate, client=client, model=args.backend,
                           temperature=args.temperature)
    print(llm_generate)
    x = task.get_input(idx)  # input
    ys = ['']  # current output candidates
    infos = []
    for step in tqdm(range(task.steps)):
        # generation
        ...
        if args.method_select == 'sample':
            ...
            select_ids = np.random.choice(
                ids, size=args.n_select_sample, p=ps).tolist()
...
def naive_solve(args, task, idx, to_print=True):
    global llm_generate
    llm_generate = partial(llm_generate, model=args.backend,
                           temperature=args.temperature)
    print(llm_generate)
    x = task.get_input(idx)  # input
